...
describe('validateResumeTailoringPlan', () => {
  ...
  it('rejects an unknown entry id', () => {
    const result = validateResumeTailoringPlan(
      plan([{ type: 'OMIT_ENTRY', entryId: 'does-not-exist', reason: 'Low relevance' }]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects two REWRITE_BULLET operations targeting the same bullet', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'A', sourceFactIds: [], requirementIds: [], reason: 'x' },
        { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'B', sourceFactIds: [], requirementIds: [], reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects REWRITE_BULLET + OMIT_BULLET on the same bullet', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'A', sourceFactIds: [], requirementIds: [], reason: 'x' },
        { type: 'OMIT_BULLET', bulletId: 'b1', reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects OMIT_ENTRY + MOVE_ENTRY on the same entry', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'OMIT_ENTRY', entryId: 'exp-1', reason: 'x' },
        { type: 'MOVE_ENTRY', entryId: 'exp-1', targetIndex: 0, reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects two MOVE_ENTRY operations targeting the same entry', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'MOVE_ENTRY', entryId: 'exp-1', targetIndex: 0, reason: 'x' },
        { type: 'MOVE_ENTRY', entryId: 'exp-1', targetIndex: 1, reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('allows multiple ADD_BULLET operations targeting the same entry', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'First addition', sourceFactIds: ['fact-1'], requirementIds: [], reason: 'x' },
        { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'Second addition', sourceFactIds: ['fact-2'], requirementIds: [], reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects an out-of-range MOVE_BULLET targetIndex', () => {
    const result = validateResumeTailoringPlan(
      plan([{ type: 'MOVE_BULLET', bulletId: 'b1', targetIndex: 5, reason: 'x' }]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('accounts for this plan\'s own omits when validating a MOVE_BULLET targetIndex', () => {
    ...
    const result = validateResumeTailoringPlan(
      plan([
        ...
        { type: 'MOVE_BULLET', bulletId: 'b1', targetIndex: 0, reason: 'x' },
      ]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects a REORDER_SKILLS with a duplicate id', () => {
    const result = validateResumeTailoringPlan(
      plan([{ type: 'REORDER_SKILLS', orderedSkillGroupIds: ['skill-1', 'skill-1'], reason: 'x' }]),
      baseResume(),
      ALLOWLISTS,
    );
  ...
  it('rejects a REORDER_SKILLS missing an existing skill group', () => {
    const result = validateResumeTailoringPlan(
      plan([{ type: 'REORDER_SKILLS', orderedSkillGroupIds: ['skill-1'], reason: 'x' }]),
      baseResume(),
      ALLOWLISTS,
    );
    expect(result).toMatchObject({ status: 'rejected', reason: 'invalid_skill_reorder' });
  ...
  it('accepts a REORDER_SKILLS that is a full permutation of existing skill groups', () => {
    const result = validateResumeTailoringPlan(
      plan([{ type: 'REORDER_SKILLS', orderedSkillGroupIds: ['skill-2', 'skill-1'], reason: 'x' }]),
      baseResume(),
      ALLOWLISTS,
    );
    expect(result.status).toBe('ok');
  ...
  it('rejects OMIT/MOVE conflicts even amid several otherwise-valid operations', () => {
    const result = validateResumeTailoringPlan(
      plan([
        { type: 'REWRITE_BULLET', bulletId: 'b3', proposedText: 'Resolved support tickets', sourceFactIds: [], requirementIds: [], reason: 'x' },
