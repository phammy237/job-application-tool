...
describe('applyResumeTailoringPlan', () => {
  it('never mutates the input base résumé', () => {
    ...
    applyResumeTailoringPlan(base, [
      { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'Changed', sourceFactIds: [], requirementIds: [], researchFindingIds: [], reason: 'x' },
  ...
  it('rewrites a bullet\'s text while preserving its id', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'New text', sourceFactIds: [], requirementIds: [], researchFindingIds: [], reason: 'x' },
    ]);
  ...
  it('sets CANDIDATE_FACTS provenance on a rewrite that cites facts', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'New text', sourceFactIds: ['fact-1'], requirementIds: [], researchFindingIds: [], reason: 'x' },
    ]);
    const bullet = proposed.experience[0]!.bullets.find((b) => b.id === 'b1');
  ...
  it('preserves the original provenance on a rewrite that cites no new facts', () => {
    ...
    const proposed = applyResumeTailoringPlan(base, [
      { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'Reworded only', sourceFactIds: [], requirementIds: [], researchFindingIds: [], reason: 'x' },
    ]);
    const bullet = proposed.experience[0]!.bullets.find((b) => b.id === 'b1');
  ...
  it('adds a new bullet with a server-generated id, never a model-supplied one', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'Added bullet', sourceFactIds: ['fact-1'], requirementIds: [], researchFindingIds: [], reason: 'x' },
    ]);
  ...
  it('moves a bullet within its own entry', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'MOVE_BULLET', bulletId: 'b3', targetIndex: 0, researchFindingIds: [], reason: 'x' },
    ]);
  ...
  it('moves an entry within its own section', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'MOVE_ENTRY', entryId: 'exp-2', targetIndex: 0, researchFindingIds: [], reason: 'x' },
    ]);
  ...
  it('reorders skill groups', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'REORDER_SKILLS', orderedSkillGroupIds: ['skill-2', 'skill-1'], researchFindingIds: [], reason: 'x' },
    ]);
  ...
  it('applies several nonconflicting operations together deterministically', () => {
    const ops: ResumeTailoringOperation[] = [
      { type: 'REWRITE_BULLET', bulletId: 'b1', proposedText: 'Rewritten', sourceFactIds: [], requirementIds: [], researchFindingIds: [], reason: 'x' },
      { type: 'OMIT_BULLET', bulletId: 'b2', researchFindingIds: [], reason: 'x' },
      { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'New', sourceFactIds: ['fact-1'], requirementIds: [], researchFindingIds: [], reason: 'x' },
  ...
  it('produces no duplicate ids anywhere after a mix of adds/omits/moves', () => {
    const proposed = applyResumeTailoringPlan(baseResume(), [
      { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'One', sourceFactIds: ['f1'], requirementIds: [], researchFindingIds: [], reason: 'x' },
      { type: 'ADD_BULLET', entryId: 'exp-1', proposedText: 'Two', sourceFactIds: ['f1'], requirementIds: [], researchFindingIds: [], reason: 'x' },
      { type: 'OMIT_BULLET', bulletId: 'b3', researchFindingIds: [], reason: 'x' },
      { type: 'MOVE_BULLET', bulletId: 'b1', targetIndex: 1, researchFindingIds: [], reason: 'x' },
    ]);
    const allIds = [
