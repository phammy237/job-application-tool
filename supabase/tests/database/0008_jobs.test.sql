...
select is(
  ...
  1,
  'user A can insert and select their own job row'
);
...
select is(
  (select count(*)::int from public.jobs where user_id = 'a0000000-0000-4000-8000-000000000001'),
  0,
  'user B cannot see user A''s job row, even though the same public posting could be analyzed by both'
);
...
select is(
  (select count(*)::int from public.jobs where user_id = 'a0000000-0000-4000-8000-000000000002'),
  1,
  'user B can independently create their own extraction record for the same posting'
);
