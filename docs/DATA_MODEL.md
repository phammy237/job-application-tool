...
Full design record: `docs/COMPANY_RESEARCH.md`. Four new tables (migration 0025), all immutable
once written (`reject_immutable_row_mutation`, except `company_research_snapshots` — see below),
all select-only for `authenticated` (no INSERT policy; every row is created exclusively through
the `create_company_research_snapshot` RPC). No `companies` table — research stays application-
contextual, with company/role frozen as plain text at research time.

| table                              | key columns                                                                                                       | notes                                                                                                                                                    |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `company_research_snapshots`        | `application_id` (nullable, composite FK `on delete set null`), `company_name`, `role_title`, `job_snapshot_id` (nullable, `set null`), `researched_at` | The one table with a *specialized* immutability trigger, not the generic one — see below.                                                                |
| `company_research_sources`          | `snapshot_id` (composite FK `on delete cascade`), `url`, `title`, `publisher`, `source_type`, `published_at` (nullable, never manufactured), `evidence_excerpt` (≤1500 chars) | Unique per `(snapshot_id, url)`. Provenance metadata only — never a page dump.                                                                            |
| `company_research_findings`         | `snapshot_id` (composite FK `on delete cascade`), `category`, `claim` (≤500 chars), `role_relevance` (nullable, ≤400 chars), `requirement_ids` (`text[]`, not a FK — see below) | |
| `company_research_finding_sources`  | `(user_id, finding_id, snapshot_id)` → `company_research_findings`, `(user_id, source_id, snapshot_id)` → `company_research_sources` | The citation join table. Both FKs are composite on `snapshot_id` too, not just `user_id` — a citation linking a finding and a source from *different* snapshots is structurally impossible, pgTAP-verified. |

`requirement_ids` is a plain `text[]`, not a foreign key: a cited requirement id is a
request-local string (a real `requirement_evidence_mappings.id`, or a synthesized fallback id like
`required-0` when no mapping exists) — the same non-FK posture Phase 7E's own résumé-tailoring
`requirementIds` field already has, for the same reason (no canonical "requirements" table exists
to reference).

**Why `company_research_snapshots` needs its own trigger function** (migration 0027, added after
migration 0025's own pgTAP suite caught this live): it is both immutable *and* the target of
`application_id`'s column-scoped `ON DELETE SET NULL` FK — deleting the application it was
attached to fires an `UPDATE` (nulling just that column), which the generic blanket
`reject_immutable_row_mutation` trigger would reject outright, making the application delete
itself fail. `reject_company_research_snapshot_mutation_except_application_unlink` allows exactly
that one transition (`application_id` non-null → null, every other column byte-for-byte unchanged)
and rejects everything else with the same error shape the generic trigger already uses — the first
table in this codebase that needed this combination.

### `create_company_research_snapshot` (Phase 7G)

The one atomic path for persisting a completed research pipeline run, mirroring
`promote_requirement_mapping_run`'s own shape (migration 0010): structural JSONB validation of
every source/finding (real ids, known enum values, every finding's `sourceIds` non-empty and
resolving to a source actually present in the same call's own `p_sources`) before any insert —
defense-in-depth against a bug in the calling TypeScript code, not against a malicious model (the
model's output was already validated and allowlist-checked in `validateCompanyResearchPlan` before
this function is ever called). Every source/finding id is generated by the caller before the call
(same pattern as Phase 7C's résumé entry ids) — this function never generates one of its own.
`security invoker`, granted only to `service_role`. Never references `submission_packets` — no
parameter, no code path reaches it.

## Job Discovery (D1–D3, migration 0029)

Full design record: `docs/JOB_DISCOVERY.md`. Two new tables, deliberately **not** following this
document's "every user-owned table has `user_id` and RLS" convention because they aren't
user-owned at all — `job_sources`/`job_catalog` are global, platform-owned data ("what jobs
currently exist on the internet"), structurally separate from the user-owned `jobs`/
`job_snapshots` system above ("what job did this user decide to analyze/apply to"). Neither
table has a `user_id` column; adding one purely to fit the RLS policy pattern above would
fabricate an owner that doesn't exist.

| table | key columns | notes |
| --- | --- | --- |
| `job_sources` | `(source_type, source_identifier)` unique, `enabled`, `crawl_interval_hours`, crawl-health columns | Global ATS board registry (`GREENHOUSE`, `LEVER`, `ASHBY` only this phase). RLS enabled, zero policies for `anon`/`authenticated` — service-role-only in both directions. |
| `job_catalog` | `(source_id, source_job_id)` unique (the real job-identity key), `content_hash`, `status` (`ACTIVE`/`POSSIBLY_CLOSED`/`CLOSED`), `consecutive_misses`, `closed_at` | Global, mutable catalog. RLS enabled, `authenticated` **select-only** (`using (true)`) since it's non-sensitive platform data a future `/discover` surface will read; no write policy for any client role — every write goes through the service-role admin client. |

Both tables reuse the pre-existing `set_updated_at` trigger. No immutability trigger — unlike
`job_snapshots`, `job_catalog` rows are deliberately mutable (a posting's content is overwritten
in place as the provider's own listing changes; freshness/history is tracked via
